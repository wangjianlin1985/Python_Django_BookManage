...
class FrontAddView(BaseView):  # 前台图书添加
    ...
    def post(self, request):
        ...
        book.bookTypeObj = BookType.objects.get(bookTypeId=request.POST.get('book.bookTypeObj.bookTypeId'))
...
class FrontShowView(View):  # 前台显示图书详情页
    def get(self, request, barcode):
        # 查询需要显示的图书对象
...
class ListAllView(View): # 前台查询所有图书
    def get(self,request):
        books = Book.objects.all()
        bookList = []
        for book in books:
